...
if ( ~isempty( dir( watermarked_images_dir ) ) )
    ...
    if (watermark_logo_fname ~= 0)
        ...
        watermark_logo = double( im2bw( im2gray( imread( watermark_logo ) ) ) );
    ...
    for key = 1:numOfKeys
        ...
        switch ( lower(attack) )
            case 'simple'
                attackedImage= imread( fullfile( pwd, '\watermarked_images\', strcat(num2str(key), '.png') ) );
    ...
    disp(['Correlation coefficient for image ', num2str(key), ': ', num2str(corr_coef(key))]);
    % disp(['Decoded correct for image ', num2str(key), ': ', num2str(decoded_correct(key))]);
    % disp(['Decoded wrong for image ', num2str(key), ': ', num2str(decoded_wrong(key))]);
